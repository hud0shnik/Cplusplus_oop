...
class CSpaceEntity {
  ...
  bool operator > (const CSpaceEntity & other) {
  ...
  friend ostream & operator << (ostream & s, CSpaceEntity & value);
};
...
ostream & operator << (ostream & s, CSpaceEntity & value) {
  s << "{ " << value.name << ", " << value.population << ", " << (int) value.type << ", " << value.icdex << ", " << value.size << " }" << " ";
  return s;
};
