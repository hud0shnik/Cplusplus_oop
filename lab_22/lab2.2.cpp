...
public:
    ...
    program() { Price = rand() % 10000; }
    program(int Price) { Price = Price; }
};

ostream& operator<<(ostream& stream, const program& serial)
{
...
int main()
{
    ...
    program Stad[3] = { s1, s2, s3 };

    auto compare = [](program& L, program& b) { return L < b; };
